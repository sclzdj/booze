...
//                ['role', '角色', 'select', RoleModel::getTree(null, false)],
    ...
    public function add()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($user = UserModel::create($data)) {
                ...
                return $this->success('新增成功', url('index'));//url('admin/agentconfig/setAgent',['uid'=>$user['id']]));
            } else {
    ...
    public function edit($id = null,$group='tab1')
    {
        ...
        $admin_id=$_SESSION['tp_admin_']['user_auth']['uid'];
        if($admin_id!=$id){
        ...
        if ($this->request->isPost()) {
            ...
            //禁止管理员修改角色
            $admin_user=db('admin_user')->find($admin_id);
            if($admin_user['role']=='2'){
                if($data['role']!='2'){
                    return $this->error('禁止修改角色');
                }
                if($data['status']!=$admin_user['status']){
                    return $this->error('禁止修改角色状态');
                }
            }
